...
script:
```sh
mkdir rdi-installer-image
# Hardlink the files into the new structure 
cp -al mkosi.images rdi-installer-image/
# Create the archive targeting the new directory
find rdi-installer-image -print0 | cpio -o -0 -H newc > mkosi-obs-include.obscpio
rm -rf rdi-installer-image
```
